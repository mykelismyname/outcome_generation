...
from transformers import (AutoConfig,
                          ...
                          TrainingArguments,
                          set_seed,
                          GPT2Tokenizer, GPT2TokenizerFast, GPT2Model, GPT2LMHeadModel, T5ForConditionalGeneration,
                          T5Tokenizer, BartForConditionalGeneration,
                          AdamW,
                          )
...
import detection_model_back as detection
...
class ExtraArguments:
    ...
    partial_contexts: Optional[bool] = field(
        default=False,
        metadata={
            "help": "train mlm with prompts having different contexts, where contexts refers to prompts of different freq occurence"},
    )
    ...
    layers: Optional[str] = field(
        default='average',
        metadata={
            "help": "either select a average of the hidden states across all model layers or select last layer hidden states"}
    )
...
def train(model, detection_model, mlm_args, train_data, eval_data, train_args, extra_args, tokenizer):
    ...
    if extra_args.trainer_api:
        ...
        train_result = trainer.train(
            resume_from_checkpoint=None) if train_args.resume_from_checkpoint is None else trainer.train(
        ...
        # model parameters
        mlm_model, mlm_criterion, optim = mlm_args
        ...
        for index in range(1):
            ...
            print("Sample {} of the training set: {}. {}, {}".format(index, train_data[index], type(train_data),
        ...
        with open(os.path.join(train_args.output_dir, 'losses.txt'), 'w') as l:
            ...
            l.write('Train_loss \t Train_Perplexity \t Val_loss \t Val_Perplexity\n')
            logging.info("\n--------------------TRAINING BEGINS--------------------\n")
            for epoch in range(int(train_args.num_train_epochs)):
                ...
                for step, batch in enumerate(training_loop):
                    ...
                    input_embs = prepare_embeddings(model=model, input_ids=input_ids, attention_mask=attention_mask,
                                                    labels=labels, layer=extra_args.layers)
                    ...
                    if extra_args.detection_loss:
                        det_loss = mlm_criterion(batch_det_probs.view(-1, batch_det_probs.size(-1)),
                                                 ner_labels.view(-1))
                ...
                logging.info(
                    "\nEpoch {}\nTraining loss: {}, Train_Perplexity: {}\nEval_loss:{} Eval_Perplexity: {}\n".format(
                        epoch + 1,
                        np.round(train_epoch_loss, 4),
                        train_perplexity,
                        np.round(eval_epoch_loss, 4),
                        eval_perplexity))
                loss_metrics['training'].append(train_epoch_loss)
                loss_metrics['val'].append(eval_epoch_loss)
                l.write('{}-{}\t{}\t{}\t{}\n'.format(epoch + 1, np.round(train_epoch_loss, 4), train_perplexity,
                                                     np.round(eval_epoch_loss, 4), eval_perplexity))

                # early stop
                loss_increment = loss_metrics['training'][-1] - loss_metrics['training'][-2] if len(
                    loss_metrics['training']) > 1 else 0.1
                ...
                if epoch == (int(train_args.num_train_epochs) - 1):
...
def evaluate(data, labels, train_args, model, tokenizer):
    ...
    with open(train_args.output_dir + '/fact_prediction.txt', 'w') as fact:
        for txt, lab in zip(data, labels):
            masked_sequence = f" ".join(
                [tokenizer.mask_token if j in labels_copy else i for i, j in zip(txt.split(), lab.split())])
                ...
                if outcomes[outcome] in mem_accuracy:
                    if extra_args.recall_metric == 'partial_match':
                        ...
                        if outcome == prediction or outcome in prediction:
                            ...
                            prompt[str(outcome_len) + '_' + str(masked_text_len) + '_Correct' + '_' + str(
                                correct_count + 1)] = masked_text
...
def main(args):
    ...
    def tokenize(examples):
        ...
        tokenized_encodings = tokenizer(examples[text_column_name]) if extra_args.pretrained_model.lower() == 'gpt2' \
            else tokenizer(examples[text_column_name], max_length=max_seq_length, truncation=True, padding='max_length',
    ...
    def add_marker_tokens(instance):
        ...
        if 'token_type_ids' in instance:
            instance['token_type_ids'].insert(1,
                                              0)  # first sequence has all it's tokens represented with 0 and second sequence are 1's
    ...
    if extra_args.fill_evaluation:
        ...
        for i in data:
            ...
            fill_evaluation(data=eval_data, labels=eval_data_labels, train_args=train_args, extra_args=extra_args,
...
if __name__ == '__main__':
    ...
    par.add_argument('--do_train', action='store_true',
                     help='call if you want to fine-tune pretrained model on dataset')
    par.add_argument('--do_eval', action='store_true',
                     help='call if you want to evaluate a fine-tuned pretrained model on validation dataset')
    par.add_argument('--do_fill', action='store_true',
                     help='Get the model to fill in masked entities in the validation dataset')
    ...
    par.add_argument('--pretrained_model', default='bert-base-uncased',
                     help='pre-trained model available via hugging face e.g. dmis-lab/biobert-v1.1')
    ...
    par.add_argument('--mask_id', default=103,
                     help='id for the special mask token 103-Bert, 50264-Roberta, 104-SciBERT')
    par.add_argument('--resume_from_checkpoint', default=None, help='location of most recent model checkpoint')
    par.add_argument('--custom_mask', action='store_true',
                     help='specify tokens to mask and avoid using the data collator')
    par.add_argument('--mention_frequency', default='outcome_occurrence.json',
                     help='File with the outcome mention frequency.')
    ...
    par.add_argument('--detection_loss', action='store_true',
                     help='include an auxiliary cross entropy loss for outcome detection')
    par.add_argument('--layers', default='average',
                     help='either select a average of the hidden states across all model layers or select last layer hidden states')
    ...
    par.add_argument('--add_marker_tokens', action='store_true',
                     help='Trigger the f_prompt  function to insert marker tokens at start of prompt')
    par.add_argument('--prompt_conditioning', default=0, type=int,
                     help='Trigger an order based attention to create a distributin')
    par.add_argument('--evaluation_strategy', default='steps', help='The evaluation strategy to adopt during training')
    par.add_argument('--label_all_tokens', action='store_true',
                     help='opt to label all tokens or not after tokenization')
    par.add_argument('--partial_contexts', action='store_true',
                     help='train mlm with prompts having different contexts, where contexts refers to prompts of different freq occurence')
    args = par.parse_args()
    main(args)
