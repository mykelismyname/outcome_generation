...
class ExtraArguments:
    ...
    prompt_type: str = field(
        default='pre',
        metadata={"help": "which prompt type to evaluate"}
    )
...
def fill_evaluation(data, labels, train_args, extra_args, model, tokenizer):
    ...
    mem_accuracy, prompt_type_accuracy = {}, {}
    facts = {}
    prompt_count = 1
    pos, pre, clo, mix = 0, 0, 0, 0
    print('\n\n\n+++++++++++++++++++++++++++++++==============================================+++++++++++++++++++++++++++++++\n\n\n')
    prompt_types = [i.strip() for i in extra_args.prompt_type.split()]
    for text, labels in zip(data, labels):
        ...
        if existing_outcomes:
            ...
            if len(existing_outcomes) == 1:
                if text.startswith(existing_outcomes[0]):
                    pos += 1
                    prompt_type = 'pos'
                    # text = "[postfix]"+text
                elif text.endswith(existing_outcomes[0]):
                    pre += 1
                    prompt_type = 'pre'
                else:
                    clo += 1
        ...
        prompt_count += 1
    print(mem_accuracy)
    eval_dir = train_args.output_dir
    if not os.path.exists(eval_dir):
        eval_dir = prepare_data.create_directory(eval_dir)
    eval_dir = prepare_data.create_directory(eval_dir+'/{}'.format(extra_args.recall_metric))
        ...
        for typ in prompt_type_accuracy:
            if extra_args.recall_metric == 'partial_match':
                ...
                if prompt_type_accuracy[typ]['Correct'] > 0 and prompt_type_accuracy[typ]['Total'] > 0:
                    prompt_type_accuracy_[typ] = float(prompt_type_accuracy[typ]['Correct']/prompt_type_accuracy[typ]['Total'])
...
def main(args):
    ...
    if extra_args.fill_evaluation:
        ...
        # for i in data:
        #     eval_data, eval_data_labels = [], []
        #     id2label = dict([(m, n) for m, n in enumerate(data[i].features['ner_tags'].feature.names)])
        #     for j in range(data[i].num_rows):
        #         ed = ' '.join(data[i]['tokens'][j])
        #         edl = ' '.join([id2label[i] for i in data[i]['ner_tags'][j]])
        #         eval_data.append(ed)
        #         eval_data_labels.append(edl)
        #     print('\n----------------------Finished Loading-------------------\n')
