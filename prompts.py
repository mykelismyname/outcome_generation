...
def prompt(args):
    ...
    try:
        mask_to_replace = [i.strip() for i in re.findall(r"\s#*\s", prompt)]
        ...
        for m in mask_to_replace:
            ...
            # all_predictions = [list(i) for i in zip(*all_predictions)]
            for i,p in enumerate(list(zip(*all_predictions))):
                print('Prediction {}:- {}'.format(i+1, ' '.join([tokenizer.decode([id]) for id in p])))
